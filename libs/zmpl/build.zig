...
pub fn buildLibrary(
    ...
    const lib = b.addStaticLibrary(.{
        ...
        .root_source_file = .{ .path = path ++ "/src/zmpl.zig" },
        .target = target,
        .optimize = optimize,
    });

    const zmpl_module = b.addModule("zmpl", .{ .root_source_file = .{ .path = path ++ "/src/zmpl.zig" } });
    ...
    ) orelse args.options.template_path orelse try std.fs.path.join(
        b.allocator,
        &[_][]const u8{ "src", "templates" },
    );
