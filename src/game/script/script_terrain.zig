pub fn evalTerrainFunc(allocator: std.mem.Allocator, luaInstance: *ziglua.Lua, buf: []const u8) ![]u32 {
    ...
    _ = luaInstance.getGlobal("chunk") catch |err| {
        std.log.err("evalTerrainFunc: failed to get global chunks. {}", .{err});
        return err;
    };
    if (luaInstance.isTable(-1) == false) {
        std.log.err("evalTerrainFunc: chunks is not a table", .{});
        return script_utils.ScriptError.ExpectedTable;
    }
    luaInstance.len(-1);
    const tableSize = luaInstance.toInteger(-1) catch |err| {
        std.log.err("evalTerrainFunc: failed to get table size", .{});
        return err;
    };
    const ts = @as(usize, @intCast(tableSize));
    luaInstance.pop(1);
    if (luaInstance.isTable(-1) == false) {
        std.log.err("evalTerrainFunc: chunks is not back to a table\n", .{});
        return script_utils.ScriptError.ExpectedTable;
    }
    ...
    for (1..(ts + 1)) |i| {
        ...
        const blockId = luaInstance.toInteger(-1) catch |err| {
            std.log.err("evalTerrainFunc: failed to get block\n", .{});
            return err;
        };
